...
| デスクトップ | 縮小 `^-` | 拡大 `^+` | 無効 | 無効 |
| 文字入力中／テキスト編集アプリ | 縮小 `^-` | 拡大 `^+` | 元に戻す `^z` | やり直し `^+z` |
...
- 元に戻す／やり直しになるのは `IsTextEditing()`（文字入力中）または `IsTextEditApp()`（メモ帳・VSCode）のとき。それ以外はタブ切り替えや `Tab` になる
- `IsTextEditing()` は**アプリの外から見える2つの状態**で判断する。どちらも Electron 系で取得できることを実測済み
  - `IsImeOn()` … 日本語入力ONなら長文を書いている最中とみなす（`WM_IME_CONTROL` / `IMC_GETOPENSTATUS`）
  - 直前の打鍵 … `InputHook` の `OnChar` で最後に印字可能文字が入力された時刻を記録し、`TYPING_WINDOW`（初期値4秒）以内なら入力中とみなす
- キャレットによる判定（`CaretGetPos()`）は**使えない**。Win32 では正しく効くが、Chrome はアドレスバーにフォーカスしていても false を返す（実測確認済み）
- `OnChar` で `Ord(char) >= 0x20` の印字可能文字だけを拾うのは、`Ctrl+A` などが制御文字（0x01等）として届くため。弾かないとコピー直後に誤って取り消しが走る
- `InputHook` には必ず `"V"`（Visible）を付ける。付けないと打鍵をアプリに渡さず飲み込んでしまう。付けた状態では入力が壊れないことを対照実験で確認済み
- 拾った文字は使わないため、`InputHook` は5分ごとに作り直して内部バッファを捨てる
