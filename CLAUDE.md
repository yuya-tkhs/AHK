...
- 対象は**英字・数字・記号すべて**。英字だけにしていたら記号のツールキー（Premiereの `:` など）が救済されなかったため広げた。記号は `^` がAHKの修飾キー記号と紛れるので、JIS配列で位置が決まる**vkコード**で指定する（`vkBA`=`:` / `vkDE`=`^` / `vkC0`=`@` など。送り直しで正しい文字になることは実測確認済み）
