...
Adobe系でIMEがONのままツールキー（`V` など）を押すと、打鍵が未確定文字列に吸われてツールが切り替わらない。これを自動で救済する。`Esc`（未確定を破棄）→ `{vk1D}`（半角英数）→ 元のキー再送、の順に送り、`MyTooltip` で何をしたか知らせる。
...
- 打鍵の直後はまだ窓が無い（出るのは実測約60ms後）ので、`AdobeImeRescueDelay`（120ms）待ってから判定する
- 判定コストは実測 0.07ms/回（`IsImeOn()` は 0.013ms）。**常駐 `InputHook` は使わない**（体感遅延が出るため廃止済み）
